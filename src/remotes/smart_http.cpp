...
void clone_http(const std::string &url, const std::string &dest_dir_str)
{
    ...
    for (const auto &ref : discovery.refs)
    {
        ...
        if (first_want)
        {
            request_body += pkt_line("want " + ref.sha + " ofs-delta agent=minigit/1.8.2\n");
            first_want = false;
        }
        else
        {
...
void fetch_http(const Repository &local, const std::string &remote_name, const std::string &url)
{
    ...
    for (const auto &w : wants)
    {
        if (first)
        {
            req += pkt_line("want " + w + " ofs-delta agent=minigit/1.8.2\n");
            first = false;
        }
        else
        {
    ...
    const std::string &branch_name)
{
    ...
    std::string req;
    req += pkt_line(old_sha + " " + local_branch_sha + " " + ref_target + '\0' + "report-status agent=minigit/1.8.2\n");
    req += pkt_flush();
