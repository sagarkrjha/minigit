...
private:
    ...
    void handle_info_refs_upload_pack(socket_handle_t client)
    {
        ...
        if (!main_sha.empty())
        {
            ...
            std::string line = main_sha + " HEAD\0symref=HEAD:refs/heads/main ofs-delta agent=minigit/1.10.0\n"s;
    ...
    void handle_info_refs_receive_pack(socket_handle_t client)
    {
        ...
        {
            using namespace std::string_literals;
            std::string line = main_sha + " refs/heads/main\0report-status agent=minigit/1.10.0\n"s;
            payload += pkt_line(line);
        }
        payload += pkt_flush();
...
TEST_CASE(SmartHttp, AdvertisedRefsParsing)
{
    ...
    std::string valid_response =
        ...
        pkt_flush() +
        pkt_line("abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890 HEAD\0symref=HEAD:refs/heads/main ofs-delta agent=minigit/1.10.0\n"s) +
